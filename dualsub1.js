/*
    ...
    - 支持 Disney+、Star+、HBO Max、Prime Video、YouTube 的官方双字幕
    - 支持 Disney+、Star+、HBO Max、Hulu、Netflix、Paramount+、Prime Video 等的外部字幕
    - 支持 Disney+、Star+、HBO Max、Hulu、Netflix、Paramount+、Prime Video 等的机器翻译双字幕（Google、DeepL）
    - 支持自定义语言设置

    使用手册：
        Shortcuts 设置工具: https://www.icloud.com/shortcuts/8ec4a2a3af514282bf27a11050f39fc2
...
let settings = {
    ...
    Hulu: {
        // 固定 Hulu 的设置为中英双语合并显示
        type: "Google", // 使用 Google 翻译
        lang: "English", // 原始语言为英语
        sl: "en", // 源语言设置为英语
        tl: "zh", // 目标语言设置为中文
        line: "s", // 合并显示中英双语
        dkey: "null", // 不使用 DeepL API 密钥
        s_subtitles_url: "null",
        t_subtitles_url: "null",
        subtitles: "null",
        subtitles_type: "null",
...
if (url.match(/action=set/)) {
    ...
    if (new_setting.type === "Reset") new_setting = settings[service];
    // 硬编码 Hulu 设置，不进行更新
    if (service !== "Hulu") {
        ...
        if (new_setting.subtitles_line) settings[service].subtitles_line = new_setting.line;
...
if (setting.type === "Official" && url.match(/\.m3u8/)) {
    ...
    if (body.match(patt)) {
        ...
        let options = { url: subtitles_data_link, headers: headers };
        $httpClient.get(options, function (error, response, data) {
            let subtitles_data = "";
